...
When there are many event receivers in the timing network, it is convenient if target delay compensation can be set for all the event receivers to the same value at the same time. In order to use this functionality, let \texttt{DC\_SOURCE} macro of runScript command in the startup script point to the record which will be used to set target delay compensation value for EVRs in the timing network. Assuming this record is \texttt{\$(SYS)-TMA:EvrDC-SP}, the runScript command will look similar to:
\begin{verbatim}
runScript $(mrfioc2_DIR)/mrfioc2_evr-VME.cmd, 
          ...
          DC_SOURCE=$(SYS)-TMA:EvrDC-SP"
\end{verbatim}
Target delay compensation of this EVR will be automatically updated to \texttt{\$(SYS)-TMA:EvrDC-SP} value, whenever it changes. In order to disable automatic update simply set \texttt{DISA} field of \newline\texttt{\$(SYS)-\$(DEVICE):DlyCompensation-Source-I} record to \texttt{1}.
