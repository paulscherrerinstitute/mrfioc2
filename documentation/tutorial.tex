...
In this document you will see URLs to reference files in the mrfioc2 git repository.
All the URLs point the master branch, but most likely you should use the latest tagged version.
You can retrieve the latest tagged version here:
\url{https://git.psi.ch/epics_driver_modules/mrfioc2/tags}
(specified on \verb!<major>.<minor>.<patch>! versioning scheme). When using the
reference files please make sure that you replace \verb!master! to
\verb!<latest_version>! in the URL.
...
\end{verbatim}	
	...
	\item 
		...
		\begin{verbatim}
			...
			cd ..
		\end{verbatim}
	\item \label{quickstart:copySubs} Copy the content of the substitution file at: \url{https://git.psi.ch/epics_driver_modules/mrfioc2/raw/master/PSI/example/evr_VME-300.subs} to newly created \texttt{cfg/EVR0.subs}. This substitution file can always be used as a starting point for new applications. Substitution files for other form factors are available at the same location (mrfioc2 repository in folder \texttt{PSI/example}).
	...
	\item 
		Mind when using EVRs which support delay compensation (eg. 300-series)! Make sure you set \texttt{DlyCompensation-Source-I} macro in \newline\texttt{cfg/EVR0.subs} to point to the record which you will use to set delay compensation target value for all the EVRs in the timing network. More details about this can be found in comments next to the macro in \texttt{cfg/EVR0.subs} or in EVR manual.
	\item
		...
		\end{verbatim}
	\item 
		Copy the content of the startup script at 
        \url{https://git.psi.ch/epics_driver_modules/mrfioc2/raw/master/PSI/example/evr_VME_startup.script} to newly created \texttt{MTEST-VME-TIMINGTEST\_startup.script}. Startup scripts for other form factors are available at the same location (mrfioc2 repository in folder \texttt{PSI/example}).
	...
	{
	  {
	    ...
	    CML0-Pwr-Sel=1,
	    CML0-Mode-Sel=0,
	    CML0-Pat_Rise00_15-SP=0x0,
	    CML0-Pat_Rise16_31-SP=0x0,
	    CML0-Pat_Rise32_39-SP=0x0,
	    CML0-Pat_High00_15-SP=0x0,
	    CML0-Pat_High16_31-SP=0x0,
	    CML0-Pat_High32_39-SP=0x0,
	    CML0-Pat_Fall00_15-SP=0x0,
	    CML0-Pat_Fall16_31-SP=0x0,
	    CML0-Pat_Fall32_39-SP=0x0,
	    CML0-Pat_Low00_15-SP=0xFFFF,
	    CML0-Pat_Low16_31-SP=0xF000,
	    CML0-Pat_Low32_39-SP=0x0,
	    ...
	  }
	}
\end{verbatim}
	...
	\begin{itemize}
		...
		\item \texttt{MTEST-VME-TIMINGTEST-EVR0:CML0-Pat\_Rise00\_15-SP}
		\item \texttt{MTEST-VME-TIMINGTEST-EVR0:CML0-Pat\_Rise16\_31-SP}
		\item \texttt{MTEST-VME-TIMINGTEST-EVR0:CML0-Pat\_Rise32\_39-SP}
		\item \texttt{MTEST-VME-TIMINGTEST-EVR0:CML0-Pat\_High00\_15-SP}
		\item \texttt{MTEST-VME-TIMINGTEST-EVR0:CML0-Pat\_High16\_31-SP}
		\item \texttt{MTEST-VME-TIMINGTEST-EVR0:CML0-Pat\_High32\_39-SP}
		\item \texttt{MTEST-VME-TIMINGTEST-EVR0:CML0-Pat\_Fall00\_15-SP}
		\item \texttt{MTEST-VME-TIMINGTEST-EVR0:CML0-Pat\_Fall16\_31-SP}
		\item \texttt{MTEST-VME-TIMINGTEST-EVR0:CML0-Pat\_Fall32\_39-SP}
		\item \texttt{MTEST-VME-TIMINGTEST-EVR0:CML0-Pat\_Low00\_15-SP}
		\item \texttt{MTEST-VME-TIMINGTEST-EVR0:CML0-Pat\_Low16\_31-SP}
		\item \texttt{MTEST-VME-TIMINGTEST-EVR0:CML0-Pat\_Low32\_39-SP}
	\end{itemize}
...
\begin{itemize}
	...
	\item \texttt{CML0-Pat\_Low00\_15-SP}=0xFFFF: Set logic low pattern bits 0-15. The Figure~\ref{fig:gen_evt_clk} shows that the bits 15-0 must be set as follows: \texttt{1111 1111 1111 1111}, which translates to \texttt{0xFFFF}. Each bit represents $1/40$ of the event clock period.
	\item \texttt{CML0-Pat\_Low16\_31-SP}=0xF000: Set signal low pattern bits 16-31.The Figure~\ref{fig:gen_evt_clk} shows that the bits 15-0 must be set as follows: \texttt{1111 0000 0000 0000}, which translates to \texttt{0xF000}. 
	\item \texttt{CML0-Pat\_Low16\_31-SP}=0x0: Set signal low pattern bits 32-39, as seen in Figure~\ref{fig:gen_evt_clk}. Note, that only bits 32-29 (lower 8 bits) can be used.
	\item \texttt{CML0-Pat\_Rise00\_15-SP}=0x0: Other patterns are not used, so they are set to 0.
	\item \texttt{CML0-Pat\_Rise16\_31-SP}=0x0: Other patterns are not used, so they are set to 0.
	\item \texttt{CML0-Pat\_Rise32\_39-SP}=0x0: Other patterns are not used, so they are set to 0.
	\item \texttt{CML0-Pat\_High00\_15-SP}=0x0: Other patterns are not used, so they are set to 0.
	\item \texttt{CML0-Pat\_High16\_31-SP}=0x0: Other patterns are not used, so they are set to 0.
	\item \texttt{CML0-Pat\_High32\_39-SP}=0x0: Other patterns are not used, so they are set to 0.
	\item \texttt{CML0-Pat\_Fall00\_15-SP}=0x0: Other patterns are not used, so they are set to 0.
	\item \texttt{CML0-Pat\_Fall16\_31-SP}=0x0: Other patterns are not used, so they are set to 0.
	\item \texttt{CML0-Pat\_Fall32\_39-SP}=0x0: Other patterns are not used, so they are set to 0.
\end{itemize}
	...
	{
	  {
	    ...
	    CML0-Pwr-Sel=1,
	    CML0-Mode-Sel=0,
	    CML0-Pat\_Low00_15-SP=0xFFC0,
	    CML0-Pat\_Low16_31-SP=0, 
	    ...
	  }
	}
\end{verbatim}
