...
\section{Configuration of pulsers switching between two different pulsers on the same output}\label{sec:2pulsers}
\begin{quote}
...
\begin{itemize}
\item generate a 40~ns wide pulse, 40~ns after each reception of event 4 in normal mode (when only the Reset Event is present).
\item generate a 40~ns wide pulse, 80~ns after each reception of event 4 in switched mode (when also the Set Event is present, eg. when MPS is asserted).
...
\section{Advanced: Trigger switching between two different pulses on the same output. (Normal and off beam operation)}\label{sec:triggerSwitching}
\begin{quote}
\textbf{This functionality is only available on event receivers that support pulser gating (eg. EVR-VME-300 series)!}
\end{quote}
...
This tutorial demonstrates how to configure an EVR without using helper template to generate pulses:
\begin{itemize}
\item 40~ns wide, with 40~ns delay on event 4 when \texttt{RF OFF beam} is not present.
\item 40~ns wide, with 80~ns delay on event 4 when \texttt{RF OFF beam} is present.
\item The pulse is outputted through front panel universal TTL output 0 (FrontUnivOut0).
\end{itemize}
%The setup is depicted in Figure~\ref{fig:2pulsers}.
