...
## Quick start (PSI)
To set up an IOC application for EVR we need to set up a startup script and a substitution file matching the timing card form factor. Suitable ones are available in the [`PSI/example`](tree/2.7.16/PSI/example) folder:

* EVG
    * example startup script ([`evg_VME_startup.script`](raw/2.7.16/PSI/example/evg_VME_startup.script))
    * example substitution files ([`evg_VME-230.subs`](raw/2.7.16/PSI/example/evg_VME-230.subs), [`evg_VME-300.subs`](raw/2.7.16/PSI/example/evg_VME-300.subs), [`evg_VME-300-fout.subs`](raw/2.7.16/PSI/example/evg_VME-300-fout.subs))
* EVR
    * example startup scripts ([`evr_VME_startup.script`](raw/2.7.16/PSI/example/evr_VME_startup.script), [`evr_PCIe_startup.script`](raw/2.7.16/PSI/example/evr_PCIe_startup.script))
    * example substitution files ([`evr_cPCI-230.subs`](raw/2.7.16/PSI/example/evr_cPCI-230.subs), [`evr_PCIe-300.subs`](raw/2.7.16/PSI/example/evr_PCIe-300.subs), [`evr_PCIe-300DC.subs`](raw/2.7.16/PSI/example/evr_PCIe-300DC.subs), [`evr_VME-230.subs`](raw/2.7.16/PSI/example/evr_VME-230.subs), [`evr_VME-300.subs`](raw/2.7.16/PSI/example/evr_VME-300.subs))
...
* copy [`PSI/example/evr_VME-300.subs`](raw/2.7.16/PSI/example/evr_VME-300.subs) to `TOP/cfg/EVR0.subs`
* configure parameters of the EVR by setting macros in `TOP/cfg/EVR0.subs`. Individual parameters are described in [`documentation/evr_manual.pdf`](raw/2.7.16/documentation/evr_manual.pdf), and tutorials for various scenarios are available in [`documentation/tutorial.pdf`](raw/2.7.16/documentation/tutorial.pdf).
* add the following to your startup script (available in [`PSI/example/evr_VME_startup.script`](raw/2.7.16/PSI/example/evr_VME_startup.script)):
...
* prepare a switable IOC structure in a `TOP` folder (where `TOP` is your project folder)
* copy [`PSI/example/evg_VME-300.subs`](raw/2.7.16/PSI/example/evg_VME-300.subs) to `TOP/cfg/EVG0.subs`
* configure parameters of the EVG by setting macros in `TOP/cfg/EVG0.subs`. Parameters are described inside the file.
* add the following to your startup script (available in [`PSI/example/evg_VME_startup.script`](raw/2.7.16/PSI/example/evg_VME_startup.script)):
...
### PSI
Example substitution files and startup scripts are available in the [`PSI/example`](tree/2.7.16/PSI/example) folder. For more details inspect the [`evr_manual.pdf`](raw/2.7.16/documentation/evr_manual.pdf) and [`tutorial.pdf`](raw/2.7.16/documentation/tutorial.pdf) available in the [`documentation`](tree/2.7.16/documentation) folder.
