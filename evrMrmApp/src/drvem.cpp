...
void
EVRMRM::setSPIData(epicsUInt8 data){
    WRITE32(base, SPIDData, (epicsUInt32)data);
}

epicsUInt8
EVRMRM::SPIData() const{
    return (epicsUInt8)READ32(base, SPIDData);
}

bool
EVRMRM::SPIOverrunError() const{
    return (READ32(base, SPIDCtrl) & SPIDCtrl_E_mask) != 0;
}
bool
EVRMRM::SPIReceiverReady() const{
    return (READ32(base, SPIDCtrl) & SPIDCtrl_RRDY_mask) != 0;
}
bool
EVRMRM::SPITransmitterReady() const{
    return (READ32(base, SPIDCtrl) & SPIDCtrl_TRDY_mask) != 0;
}
bool
EVRMRM::SPITransmitterEmpty() const{
    return (READ32(base, SPIDCtrl) & SPIDCtrl_TMT_mask) != 0;
}
bool
EVRMRM::SPITransmitterOverrun() const{
    return (READ32(base, SPIDCtrl) & SPIDCtrl_TOE_mask) != 0;
}
bool
EVRMRM::SPIReceiverOverrun() const{
    return (READ32(base, SPIDCtrl) & SPIDCtrl_ROE_mask) != 0;
}
void
EVRMRM::setSPIOutputEnable(bool oe){
    if(oe)
        BITSET(NAT,32,base, SPIDCtrl, SPIDCtrl_OE_mask);
    else
        BITCLR(NAT,32,base, SPIDCtrl, SPIDCtrl_OE_mask);
}

bool
EVRMRM::SPIOutputEnable() const{
    return (READ32(base, SPIDCtrl) & SPIDCtrl_OE_mask) != 0;
}
void
EVRMRM::setSPISlaveSelect(bool oe){
    if(oe)
        BITSET(NAT,32,base, SPIDCtrl, SPIDCtrl_SSO_mask);
    else
        BITCLR(NAT,32,base, SPIDCtrl, SPIDCtrl_SSO_mask);
}

bool
EVRMRM::SPISlaveSelect() const{
